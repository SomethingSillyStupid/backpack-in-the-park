...
async def _readline_bounded(reader, max_bytes):
  line = bytearray()
  while len(line) < max_bytes:
    ...
    if chunk == b"\n":
      return bytes(line)
  raise ValueError("Request line or header is too long")
...
async def _parse_headers(reader):
  ...
  for _ in range(MAX_HEADER_COUNT):
    header_line = await _readline_bounded(reader, MAX_HEADER_LINE_BYTES)
...
async def _handle_request(reader, writer):
  ...
  try:
    request_line = await _readline_bounded(reader, MAX_REQUEST_LINE_BYTES)
    ...
    logging.error(e)
    response = ("Bad request", 400, "text/plain")

  if response is None:
    response = request_body_error(request.headers)

  if response is None and "content-length" in request.headers and "content-type" in request.headers:
    ...
    if request.headers["content-type"].startswith("application/x-www-form-urlencoded"):
      ...
      while content_length > 0:
        ...
        form_data += data
      response = urlencoded_body_error(form_data, int(request.headers["content-length"]))
      if response is None:
